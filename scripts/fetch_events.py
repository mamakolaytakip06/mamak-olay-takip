...
("Telegram","site:t.me Mamak Ankara son dakika")]
...
def add_feed(url,platform,now,out):
 try:
  ...
  for x in root.findall(".//item"):
   ...
   if not dt or now-dt>timedelta(days=365) or "mamak" not in (title+" "+desc).lower():continue
   cat,icon=classify(title+" "+desc);src=x.find("source");source=src.text if src is not None and src.text else platform
   out.append({"category":cat,"icon":icon,"title":title,"location":"Mamak / Ankara","published":dt.isoformat(),"confidence":75 if platform=="Haber" else 60,"sources":1,"status":"Muhtemel" if platform=="Haber" else "Sosyal medya / doğrulanmamış","summary":source+" üzerinden bulunan herkese açık kayıt.","url":link,"platform":platform})
...
with open("data/events.json","w",encoding="utf-8") as f:json.dump({"updated_at":now.isoformat(),"events":items,"sources":["Google News RSS","X (indekslenen açık gönderiler)","Facebook (indekslenen açık sayfa/gruplar)","Instagram","YouTube","TikTok","Threads","Telegram"]},f,ensure_ascii=False,indent=2)
