...
def load_ankara_neighborhood_catalog(now):
 ...
 try:
  ...
  if cached.get("catalog_version")==3 and now-fetched<timedelta(days=30) and len(cached.get("districts",{}))>=20:
 ...
 try:
  official_derived_url="https://raw.githubusercontent.com/bertugfahriozer/il_ilce_mahalle/master/il_ilce_mahalle.json"
  req=urllib.request.Request(official_derived_url,headers={"User-Agent":"AnkaraOlayTakip/3.3"})
  official_derived=json.loads(urllib.request.urlopen(req,timeout=20).read().decode("utf-8"))
  ankara=next((value for key,value in official_derived.items() if ascii_text(key)=="ankara"),{})
  for expected in ANKARA_DISTRICT_SLUGS:
   extra=next((names for name,names in ankara.items() if ascii_text(name)==ascii_text(expected)),[])
   existing=catalog.get(expected,[])
   combined={ascii_text(name):name for name in existing}
   for value in extra if isinstance(extra,list) else []:
    name=re.sub(r"\s+(Mah\.?|Mahallesi)$","",str(value),flags=re.I).strip().title()
    if name:combined.setdefault(ascii_text(name),name)
   if combined:catalog[expected]=sorted(combined.values(),key=ascii_text)
  source="ABB + resmî listeden türetilmiş açık katalog doğrulaması"
 except Exception:pass
 catalog["Mamak"]=list(MAMAK_NEIGHBORHOOD_VARIANTS)
 if len(catalog)>=20:
  payload={"catalog_version":3,"updated_at":now.isoformat(),"source":source,"districts":catalog}
